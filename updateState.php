...
<html>
<head>
    <title>State Update Status Page</title>
</head>
<body>
<h1>Status of state update:</h1>
<div>
    <?php
    ...
    if(!($statement = $mysqli->prepare(" UPDATE states SET states.population = ? WHERE states.stateID = ?"))){
        echo "Prepare failed: "  . $statement->errno . " " . $statement->error;
    }

    if(!($statement->bind_param("ii", $_POST['statePopulation'], $_POST['stateToUpdate']))){
